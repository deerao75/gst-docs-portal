"""
Populate list details and paragraph summaries for all orders (no API).
Run: python scripts/populate_order_summaries.py
     python scripts/populate_order_summaries.py --force
"""
...
from summarize_order import build_order_list_detail, build_order_paragraph_summary
...
def main() -> None:
    ...
    for doc in docs:
        if doc.get("doc_type") != "order" or doc.get("status") != "ready":
            continue
        ...
        order_no = doc.get("notification_no") or ""
        ...
        summary = build_order_paragraph_summary(
            text,
            order_no,
            category,
