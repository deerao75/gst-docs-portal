...
function isCorrigendum(doc: PdfDocument): boolean {
  return /corrig/i.test(doc.file_name);
}
...
function notificationNumber(doc: PdfDocument): number {
  const match = doc.notification_no?.match(/^(\d+)\//);
  return match ? parseInt(match[1], 10) : 0;
}

function compareParents(a: PdfDocument, b: PdfDocument): number {
  const numDiff = notificationNumber(b) - notificationNumber(a);
  if (numDiff !== 0) return numDiff;
  return (b.issued_date ?? "").localeCompare(a.issued_date ?? "");
}

/** Latest notification first; corrigenda immediately after their parent. */
export function sortNotifications(docs: PdfDocument[]): PdfDocument[] {
